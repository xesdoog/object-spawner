...
object_spawner:add_text("Choose Spawning Distance or Leave Default :")
...
object_spawner:add_text("Choose Facing Direction or Leave Default :")

object_spawner:add_imgui(function()
offset, isChanged = ImGui.InputInt("Facing Direction", offset, 15, 90, 0)
end)

object_spawner:add_button("Reset Direction", function()
    offset = 0
    isChanged = false
end)
...
local object = filteredItems[prop_index+1]
    if ImGui.Button("Spawn Object") then
        script.run_in_fiber(function()
            ...
            script.register_looped("change direction", function()
                if isChanged then
                ENTITY.SET_ENTITY_HEADING(prop, heading + offset)
                end
            end)
